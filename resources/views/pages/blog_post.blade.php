      ...
      <div class="content blog blog-post col-sm-9 col-md-9">
		...
		<article class="post">
		  ...
		  <div class="entry-content">
			...
			<?php if($post[0]->post_files) {?><img src="{{ url() }}<?php echo '/images/admin/blog/'.$post[0]->post_files; ?>" class="img-fluid"><?php } ?>
		  	</div>
		  </div>
		  <footer>
		...
		<ul class="commentlist">
		 	<?php foreach($postComments as $key=>$value) {?>
		  	<li>
				...
				<img class="avatar" width="50" height="50" src="{{ url() }}/<?php echo "images/profile/".$value['comments']->avatar; ?>" alt="">
				<div class="comments-meta">
				  ...
				  <span class="time"><i class="fa fa-calendar"></i> <?php echo date("F d, Y, h:m A",strtotime($value['comments']->comment_date)); ?></span>
					...
					if( isset($loggedInUserId) ):
						...
						<a href="{{ url() }}/delete_post_comment?id=<?php echo $loggedInUserId; ?>&type=blog-post&commentId=<?php echo $value['comments']->comment_ID; ?>" onclick="return confirm('Are you sure');" class="remove-comment-btn pull-right glyphicon glyphicon-remove"></a> 
					<?php }//end of if
						endif;
					?>
				</div>
					...
					if( count($value['comments_comments']) > 0 ){
						...
						<li>
							<?php 
				 			...
				 			?>
				 			<img class="avatar" width="50" height="50" src="{{ url() }}<?php echo "images/profile/".$val->avatar; ?>" alt="">
							<div class="comments-meta">
		...
		<form class="comments-form" method="post" action="{{ url() }}/add_comment_to_blog_post">
