...
class NotificationController extends Controller {
	...
	public function getIndex( Request $request )
	{
		...
		$filter = " AND userid = '".\Session::get('uid')."' ";	
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['id'] = $id;
			\DB::table('tb_notification')->where('id',$id)->update(array('is_read'=>'1'));
			$this->data['access']		= $this->access;
			$this->data['subgrid']	= (isset($this->info['config']['subgrid']) ? $this->info['config']['subgrid'] : array()); 
			return view('notification.view',$this->data);
		} else {
			return Redirect::to('notification')->with('messagetext','Record Not Found !')->with('msgstatus','error');					
		}
	}	
