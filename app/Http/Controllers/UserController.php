...
class UserController extends Controller {
	...
	public function postRegister( Request $request) {
		...
		if( User::isUsernameAvailable( trim( $request->input('username') ) ) == false  ){
			...
			if($request->ajax() == true ){
				...
				return Redirect::to('user/login')->with( 'message',\SiteHelpers::alert('error','Username already taken!') )->withErrors($validator)->withInput();	
			}
		...
		if ($validator->passes()) {
			...
			{ 
				...
				//update session data
				\Session::put('uid', $userData->id);
				\Session::put('eid', $userData->email);
				\Session::put('ll', $userData->last_login);
				\Session::put('fid', $userData->first_name.' '. $userData->last_name);	
				if(\Session::get('lang') =='')
				{
			...
			if($request->ajax() == true )
			{
				...
				return Redirect::to('user/login')->with('message',\SiteHelpers::alert('error','The following errors occurred') )->withErrors($validator)->withInput();	
			}
	...
	public function postSignin( Request $request) {
		...
		//rules consist of the input fileds with corresponding name 
		...
		if ($validator->passes()) {	
			...
			if (\Auth::attempt(array('username'=>$request->input('username'), 'password'=> $request->input('password'), 'group_id'=>$request->input('group_id') ), $remember )) {
				if(\Auth::check()){
	...
	public function getLogout() {
		...
		$logId = \DB::table('tbl_login_logs')->select('lg_id')->where('lg_user_id', '=', \Session::get('uid') )->orderBy('lg_id', 'desc')->first();
		$log_id = $logId->lg_id;
		if( $log_id !== null)
			$sql = \DB::table('tbl_login_logs')->where('lg_id', $log_id)->update( 										array('lg_logout_time' => time()) );
